...
export const FloatingPanelRoot = function FloatingPanelRoot(props) {
  const anchorPos = React.useRef({ x: 0, y: 0 })
  const fullSize = React.useRef(null)
  const restoreSize = React.useRef(null)
  const isMinimized = React.useRef(false)

  const { onStageChange, onSizeChange, onOpenChange, ...rest } = props

  const getAnchorPosition = React.useCallback((details) => {
